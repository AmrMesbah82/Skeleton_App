...
import '../../../../external/calender/services/calendar_event_model.dart';
...
class RoleManagementCalendarService {
  ...
  Future<List<CalendarEventModel>> getRoleManagementCalendarEvents({
    ...
    debugPrint('\n🗓️  ═══════════ ROLE MANAGEMENT CALENDAR EVENTS ═══════════');
    debugPrint('📧 Current user: $currentUserEmail');

    if (currentUserEmail.isEmpty) {
      debugPrint('❌ Empty email — aborting');
      return [];
    }
    ...
    try {
      ...
      debugPrint('✅ Total role management calendar events: ${events.length}');
      debugPrint('🗓️  ═══════════════════════════════════════════════════════\n');
      return events;
    } catch (e, st) {
      debugPrint('❌ FATAL ERROR in getRoleManagementCalendarEvents: $e');
      debugPrint('$st');
      return [];
    }
  }
  ...
  Future<void> _loadUsersAccessEvents({
    ...
    try {
      ...
      debugPrint('📊 Users_Access docs: ${snapshot.docs.length}');
      ...
      for (final doc in snapshot.docs) {
        try {
          ...
          debugPrint('⚠️ Error processing Users_Access doc ${doc.id}: $e');
        }
      }
    } catch (e) {
      debugPrint('❌ Error loading Users_Access events: $e');
    }
  }
  ...
  Future<void> _loadEmployeeScheduledEvents({
    ...
    try {
      ...
      debugPrint('📊 Employees_Info docs scanned: ${snapshot.docs.length}');
      ...
      for (final doc in snapshot.docs) {
        try {
          ...
          debugPrint('⚠️ Error processing employee doc ${doc.id}: $e');
        }
      }
    } catch (e) {
      debugPrint('❌ Error loading employee scheduled events: $e');
    }
  }
