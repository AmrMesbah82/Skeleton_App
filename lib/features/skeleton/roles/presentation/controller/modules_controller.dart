  ...
  import 'package:demo_app/features/skeleton/roles/domain/interfaces/module_permissions_sections.dart';
  ...
  import '../../data/models/module_permission_model.dart';
  import '../../domain/entity/access_data.dart';
  import '../../domain/entity/module_item.dart';
  import '../../utils/constants.dart';
  import '../ui/pages/role_responsive_page.dart';
  ...
  class ModulesController extends GetxController {
    ...
    Future<Map<String, bool>> getFilteredDefaultPermissionsForModule(String moduleName) async {
      print("\n🔍 ════════════════════════════════════════");
      print("🔍 getFilteredDefaultPermissionsForModule called");
      print("🔍 Module Name: $moduleName");

      Map<String, bool> allPermissions = getDefaultPermissionsForModule(moduleName);

      print("📋 Total permissions from ENUM: ${allPermissions.length}");

      try {
        String companyId = _getCompanyId();

        print("\n🔍 Step 1: Getting company ID");
        ...
        print("\n🔍 Step 2: Loading Demo_Permissions...");
        ...
        print("🔍 Demo_Permissions document exists: ${demoPermDoc.exists}");
        print("🔍 Document path: Demo_Permissions/$companyId");

        if (!demoPermDoc.exists) {
          print("⚠️ Demo_Permissions document not found!");
          return {};
        }

        Map<String, dynamic> demoPermissions = demoPermDoc.data() as Map<String, dynamic>;

        print("   Demo_Permissions keys: ${demoPermissions.keys.toList()}");

        if (!demoPermissions.containsKey(moduleName)) {
          print("⚠️ Module '$moduleName' not in Demo_Permissions");
          return {};
        }

        if (demoPermissions[moduleName] is! Map) {
          print("⚠️ Demo_Permissions[$moduleName] is not a Map");
          return {};
        }
        ...
        print("   Module has ${modulePermissions.keys.length} permissions in Demo_Permissions");

        if (allPermissions.isEmpty) {
          print("\n✅ ENUM IS EMPTY - Using RAW Demo_Permissions");
          ...
          print("✅ Returning ${rawPermissions.length} RAW permissions");
          print("   Keys: ${rawPermissions.keys.take(5).toList()}");
        ...
        print("\n✅ Returning $allowedCount filtered permissions");
    ...
    String _getCompanyId() {
      try {
        ...
        print("🔍 getBaseUrl('') returned: '$baseUrl'");
        ...
        print("🔍 Extracted company ID: '$companyId'");
        ...
        if (parts.length >= 2) {
          ...
          print("🔍 Company ID from split: '$companyId'");
          if (companyId.isNotEmpty) {
            return companyId;
          }
        }

        print("❌ Could not extract company ID from: '$baseUrl'");
        return '';
      } catch (e) {
