...
import 'package:demo_app/features/skeleton/roles/domain/use_cases/delete_user_permission_use_case.dart';
...
import '../../domain/use_cases/update_selected_members_use_case.dart';
...
class UserManagementAccessCubit extends Cubit<UserManagementAccessState> {
  ...
  void filterEmployeesToGiveAccess(String searchText) {
    print("════════════════════════════════════════════════════════");
    print("🔍 [CUBIT] filterEmployeesToGiveAccess - START");
    print("════════════════════════════════════════════════════════");
    print("   - Search text: '$searchText'");
    print("   - employeeToGiveAccess count: ${employeeToGiveAccess.length}");

    filteredEmployeeToGiveAccess = [];

    // ✅ Check current employee email
    print("🔍 [CUBIT] Checking MainCoreEmployeeController...");
    final mainCoreController = Get.find<MainCoreEmployeeController>();
    print("   - MainCoreEmployeeController found: ${mainCoreController != null}");
    print("   - employeeEntity: ${mainCoreController.employeeEntity}");
    print("   - employeeEntity?.email: ${mainCoreController.employeeEntity?.email}");
    final currentEmployeeEmail = mainCoreController.employeeEntity?.email;
    print("   ✅ currentEmployeeEmail: $currentEmployeeEmail");

    // ✅ Check company email step by step
    print("🔍 [CUBIT] Checking CompanyController...");
    try {
      final companyController = Get.find<CompanyController>();
      print("   - CompanyController found: ${companyController != null}");
      print("   - company: ${companyController.company}");
      print("   - company?.email: ${companyController.company?.email}");
      print("   - company?.email?.emails: ${companyController.company?.email?.emails}");
      print("   - company?.email?.emails?.length: ${companyController.company?.email?.emails?.length}");

      final companyEmail = companyController.company?.email?.emails?.lastOrNull;
      print("   ✅ companyEmail (lastOrNull): $companyEmail");

      print("🔍 [CUBIT] Processing ${employeeToGiveAccess.length} employees...");

      for (int i = 0; i < employeeToGiveAccess.length; i++) {
        var element = employeeToGiveAccess[i];

        if (i < 3) {
          print("   [$i] Processing: ${element.englishName}");
          print("       - arabicName: ${element.arabicName}");
          print("       - englishName: ${element.englishName}");
          print("       - employeeEmail: ${element.employeeEmail}");
          print("       - departmentId: ${element.departmentId}");
        }
        ...
        if (i < 3) {
          ...
          print("       - matchesDepartment: $matchesDepartment");
        }
        ...
        if (matchesSearch && matchesDepartment) {
          ...
          if (i < 3) {
            print("       - isCurrentEmployee: $isCurrentEmployee");
            print("       - isCompanyEmail: $isCompanyEmail");
          }

          if (isCurrentEmployee || isCompanyEmail) {
            if (i < 3) print("       ⏭️  SKIPPED (is current user or company email)");
            continue;
          }

          filteredEmployeeToGiveAccess.add(element);
          if (i < 3) print("       ✅ ADDED to filteredEmployeeToGiveAccess");
        } else {
          if (i < 3) print("       ⏭️  SKIPPED (doesn't match filters)");
        }
      }

      print("════════════════════════════════════════════════════════");
      print("✅ [CUBIT] filterEmployeesToGiveAccess - SUCCESS");
      print("   - Final filtered count: ${filteredEmployeeToGiveAccess.length}");
      print("════════════════════════════════════════════════════════");
      ...
      print("════════════════════════════════════════════════════════");
      print("❌❌❌ [CUBIT] ERROR in filterEmployeesToGiveAccess ❌❌❌");
      print("Error: $e");
      print("Stack trace:\n$stackTrace");
      print("════════════════════════════════════════════════════════");
      rethrow;
  ...
  updateSelectedMembersAccess() async {
    print('\n🎮 [CUBIT] updateSelectedMembersAccess - START');
    print('   Selected role: $newAccessSelectedRole');
    print('   Selected users: ${selectedUsersIdToChangeAccess.length}');
    ...
    if (result.isRight()) {
      ...
      Failure failure = result.fold((l) => l, (r) => null)!;
      print('   ❌ Assignment failed: ${failure.errMessage}');
