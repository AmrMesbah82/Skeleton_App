...
import '../../data/models/users_access_model.dart';

class NewPermissionEntity{
...
class UserPermissionHistoryModel {
  ...
  factory UserPermissionHistoryModel.fromMap(Map<String, dynamic> map) {
    try {
     ...
     // print("Employee_Id: ${map[EMPLOYEE_ID]}");
      ...
      print("❌ ERROR parsing UserPermissionHistoryModel: $e");
      print("Stacktrace:\n$stack");
      rethrow;
    }
  }
  ...
  UserPermissionHistoryModel copyWith({
    ...
    print("\n=== UserPermissionHistoryModel.copyWith DEBUG ===");
    int newTimestamp = DateTime.now().millisecondsSinceEpoch;

    print("Creating synchronized update at index ${timestamps.length}");
    print("Edited fields:");
    if (role != null) print("  - role: '$role'");
    if (fromDate != null) print("  - fromDate: '$fromDate'");
    if (toDate != null) print("  - toDate: '$toDate'");
    if (editBy != null) print("  - editBy: '$editBy'");
    ...
    print("  - role length: ${result.role.length}");
    print("  - fromDate length: ${result.fromDate.length}");
    print("  - toDate length: ${result.toDate.length}");
