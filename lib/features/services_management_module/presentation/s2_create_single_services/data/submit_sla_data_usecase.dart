...
// Use case classes - define here or import from your domain layer
class SubmitSlaDataUseCase {
  final dynamic repository; // Replace with your actual ServicesRepository type

  SubmitSlaDataUseCase(this.repository);
  ...
  Future<void> call({
    ...
    // return await repository.submitSlaData(...);
  }
}
...
class SaveDraftUseCase {
  ...
  Future<void> call(ServicesHistoryModel model) async {
    // Call your repository method here
    // return await repository.saveDraft(model);
  }
}

class SlaNotificationCubit extends Cubit<SlaNotificationState> {
  final SubmitSlaDataUseCase submitSlaDataUseCase;
  final SaveDraftUseCase saveDraftUseCase;

  SlaNotificationCubit({
    required this.submitSlaDataUseCase,
  ...
  // Renamed to avoid conflict with local variable in validateNotificationSettings
  bool checkProviderSlaData() {
  ...
  String? validateNotificationSettings(BuildContext context, String requesterTitle, String providerTitle, String managerTitle) {
    ...
    // Use renamed method here
    bool providerHasSlaData = checkProviderSlaData();

    if ((hasProviderSwitchEnabled || providerHasSlaData) && !model.notifyProviderChecked) {
    ...
    // Use renamed method here
    bool managerHasSlaData = checkManagerSlaData();

    if ((hasManagerSwitchEnabled || managerHasSlaData) && !model.notifyManagerChecked) {
  ...
  Future<void> handleSaveForLater(BuildContext context, ServicesHistoryModel? editingModel, VoidCallback onNavigate) async {
    ...
    if (isDraft || editingModel != null) {
      ...
      await _saveDraftData();
      emit(const SlaNotificationDraftSaved()); // ✅ Use dedicated state
      emit(SlaNotificationSaved());
    }
  }
