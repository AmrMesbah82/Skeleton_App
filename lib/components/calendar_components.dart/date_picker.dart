...
class _DualIOSStyleTimePickerState extends State<DualIOSStyleTimePicker> {
  ...
  List<TimeOfDay> selectedTimes = [TimeOfDay.now()]; // List to store selected times
  bool showSecondRow = false; // State variable to track the visibility of the second row
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
                          ...
                          builder: (BuildContext builder) {
                            return Container(
                              ...
                              child: CupertinoDatePicker(
                                ...
                                onDateTimeChanged: (DateTime newDateTime) {
                                  setState(() {
                                    selectedTimes[0] = TimeOfDay.fromDateTime(newDateTime);
                                  });
                                },
                              ),
                            );
            ...
            GestureDetector(
              onTap: () {
                if (selectedTimes.length < 4) {
                  setState(() {
                    selectedTimes.add(TimeOfDay.now()); // Add new selected time to the list
                  });
                } else {
                  showDialog(
                    context: context,
                    builder: (BuildContext context) {
                      return AlertDialog(
                        ...
                        content: Text('You have reached the maximum limit of 5 rows.'),
                        actions: [
                          TextButton(
                            onPressed: () {
        ...
        for (int i = 1; i < selectedTimes.length; i++) // Loop through selectedTimes list
          Row(
                          ...
                          builder: (BuildContext builder) {
                            return Container(
                              ...
                              child: CupertinoDatePicker(
                                ...
                                onDateTimeChanged: (DateTime newDateTime) {
                                  setState(() {
                                    selectedTimes[i] = TimeOfDay.fromDateTime(newDateTime);
                                  });
                                },
                              ),
                            );
