...
import 'package:demo_app/features/services_mangment_module/Category/data/entities/services_history_model.dart';
...
}) async
{
  ...
  if (pathParts.length == 3) {
    ...
    print('❌ Unexpected path format: $requestServicesPath');
...
Future<Map<String, dynamic>?> selectServiceProvider(String currentServiceDocId) async {
  ...
  try {
    print("🔍 Looking for service: $currentServiceDocId");
    ...
    if (pathParts.length == 3) {
      ...
      print("❌ Unexpected path format: $createServicesPath");
      return null;
    }
    ...
    if (!currentServiceDoc.exists) {
      print("❌ Service not found: $currentServiceDocId");
      return null;
    }
    ...
    if (currentProviderServices == null) {
      print("❌ No providers for service: $currentServiceDocId");
      return null;
    }
    ...
    if (parsedCurrentProviders.length == 1) {
      ...
      if (provider is Map<String, dynamic>) {
        print("✅ Single provider: ${provider['email']}");
        return provider;
      }
      return null;
    }
    ...
    print("📊 Multiple providers (${parsedCurrentProviders.length}), calculating workload...");
    ...
    print("📊 Total CreateServices docs found: ${allServicesSnapshot.docs.length}");
    ...
    print("📊 Workload calculated:");
    emailTotalDuration.forEach((email, duration) {
      print("  $email: ${duration}min (${emailFrequency[email]} services)");
    });
    ...
    print("✅ Selected provider (least busy): $selectedEmail");
    print("   Total workload: ${emailTotalDuration[selectedEmail] ?? 0}min");
    print("   Service count: ${emailFrequency[selectedEmail] ?? 0}");

    return selectedProvider;

  } catch (e, stackTrace) {
    print("❌ ERROR in selectServiceProvider: $e");
...
{
  print('');
  print('🎯 ========== selectServiceProviderApproval CALLED ==========');
  print('🎯 Request ID: $currentServiceDocId');
  print('🎯 Parameters:');
  print('   - lookbackDays: $lookbackDays');
  print('   - rotateWithinSameParent: $rotateWithinSameParent');
  print('   - respectExistingAssignment: $respectExistingAssignment');
  print('🎯 =========================================================');
  print('');
  ...
  try {
    ...
    print('📍 Searching in collection: $tenantRoot');
    print('📍 Looking for document: $currentServiceDocId');
    ...
    if (!currentDocSnapshot.exists) {
      print('❌ Document not found at path: ${currentDocRef.path}');
      return null;
    }
    ...
    print('📋 Parent Service ID: $currentParent');

    var currentProviders = _providersFromModel(currentModel);
    print('📋 Available providers: ${currentProviders.length}');

    if (currentProviders.isEmpty) {
      print('⚠️  No providers for this service');
      return null;
    }

    // Log provider details
    for (var i = 0; i < currentProviders.length; i++) {
      print('   Provider $i: ${currentProviders[i]['email']} - ${currentProviders[i]['firstName']} ${currentProviders[i]['lastName']}');
    }

    // Check existing assignment if respectExistingAssignment is true
    if (respectExistingAssignment) {
      final existingAssigned = _assignedEmailFromModel(currentModel);
      print('🔍 Checking existing assignment: $existingAssigned');
      ...
      if (existingAssigned.isNotEmpty) {
        ...
        print('✅ Using existing assigned provider: $existingAssigned');
        print('   Provider details: ${existingProvider['firstName']} ${existingProvider['lastName']}');
        return existingProvider;
      } else {
    ...
    if (currentProviders.length == 1) {
      ...
      print('✅ Only one provider, assigned: $email');
      return only;
    }
    ...
    print('📊 Calculating workload for providers: $thisServiceProviderEmails');
    ...
    print('📊 Fetching all requests from: $tenantRoot');
    final allRequestsSnapshot = await firestore.collection(tenantRoot).get();
    print('📊 Found ${allRequestsSnapshot.docs.length} total requests');
    ...
    for (final email in thisServiceProviderEmails) {
      print('  $email: ${emailTotalMinutes[email]?.toStringAsFixed(1)}min, ${emailFrequency[email]} requests');
    }
    ...
    print('✅ Selected provider: $chosenEmail');
    print('   Provider: ${chosen['firstName']} ${chosen['lastName']}');
    print('   Workload: ${emailTotalMinutes[chosenEmail]?.toStringAsFixed(1)}min, ${emailFrequency[chosenEmail]} requests');

    // Update with clean email
    await currentDocRef.update({'assignedProviderEmail': [chosenEmail]});

    print('🎯 ========== selectServiceProviderApproval SUCCESS ==========');
    print('');
    ...
    print('🎯 ========== selectServiceProviderApproval FAILED ==========');
    print('');
    return null;
  }
}
